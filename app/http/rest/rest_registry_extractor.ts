...
export function findRestDelegation(body: string): RestDelegation | undefined {
  const match = body.match(
    /\bthis(?:\.\w+)*\s*\.\s*([A-Za-z_$][\w$]*)\s*\.\s*endpoints\s*\.\s*([A-Za-z_$][\w$]*)/
  )
 ...
 */
export function collectImportNames(
  sourceText: string,
  fileName = 'fixture.ts'
): Map<string, string> {
...
export function findEndpointsLiteral(sourceText: string): ts.ObjectLiteralExpression | undefined {
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isPropertyDeclaration(node) || ts.isPropertyAssignment(node)) {
      ...
      const isEndpoints =
        (ts.isIdentifier(nameNode) || ts.isStringLiteral(nameNode)) && nameNode.text === 'endpoints'
 ...
 */
export function findValidatorInitializer(
  endpointInit: ts.ObjectLiteralExpression
): ts.Node | undefined {
 ...
 */
export function resolveValidatorImport(
  sourceText: string,
  identifier: string
): ExtractedValidator | undefined {
