...
import { buildCmsMaintenanceIndexPayload } from '#helpers/i18n_payloads/maintenance_index'

/**
 * Maintenance cms controller for admin UI (CMS area).
...
export default class MaintenanceController {
  ...
  /**
   * Render maintenance cms page (Inertia).
   */
  async render({ inertia }: HttpContext) {
    ...
    return inertia.render('maintenance/cms/index', {
      config,
      ...
      translations: buildCmsMaintenanceIndexPayload(this.i18n),
    })
  }
  ...
  async update({ request, response, session }: HttpContext) {
    ...
    try {
      ...
      session.flash(
        ...
        error instanceof RangeError
          ? error.message
          : 'Maintenance configuration could not be saved.'
      )
    ...
    session.flash('success', 'Maintenance cms saved.')
    return response.redirect().back()
  }
