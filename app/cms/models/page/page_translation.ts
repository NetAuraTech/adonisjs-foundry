...
import type { PageContent, PageStatus, ResolvedPageContent } from '#cms/types/page';
...
export default class PageTranslation extends PageTranslationSchema {
	...
	async saveRevision(createdBy: number, keepCount = 10): Promise<void> {
		const { PageRevisionRepository } = await import('#cms/domain/repositories/page/page_revision_repository');
