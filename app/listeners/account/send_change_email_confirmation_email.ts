...
export default class SendChangeEmailConfirmationEmail extends BaseTokenListener {
	...
	protected buildMailPayload(event: InitiateEmailChange, locale: string, token: string): Record<string, any> {
		return {
			user: { email: event.user.pendingEmail!, locale },
			confirmation_link: `${env.get('APP_URL')}${routePath('settings.email_change.render', { token }) ?? ''}`,
		};
	}
