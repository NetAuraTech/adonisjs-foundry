...
- **API**: no `render()`; methods return JSON via `response.ok()`, `response.badRequest()`. Serialize with the domain's transformers (`#app/{domain}/transformers/...`) + `ctx.serialize()`.
...
- Validation: import from the domain's validators (`#app/{domain}/validators/...`) or the shared core validators (`#app/core/validators/...`, e.g. the pagination schema), validate before every service call.
