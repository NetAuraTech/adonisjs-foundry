...
export function FileAltEditor(props: FileAltEditorProps) {
	...
	const handleSubmit = async (e: SubmitEvent<HTMLFormElement>, callback: (value: boolean) => void) => {
		const data = Object.fromEntries(new FormData(e.target)) as {
			locale: string;
			key: string;
			value: string;
		...
		setAlts((currentAlts) => {
			...
			if (exists) {
				return currentAlts.map((alt) => (alt.locale === data.locale && alt.key === data.key ? data : alt));
			}
