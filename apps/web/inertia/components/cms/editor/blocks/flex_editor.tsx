...
export function FlexEditor({ block, onChange, lockProps }: EditorProps) {
	...
	return (
		<div className="space-y-4">
			<LFW {...lockProps} fieldKey="as" label="As" type="select" defaultValue={p.as} onChange={(v) => u('as', v)}>
			...
			<ResponsiveControl label="Direction" value={p.direction} onChange={(v) => u('direction', v)}>
				{(val, bp, update) => (
					<LFW {...lockProps} fieldKey={`direction.${bp}`} label="" type="select" defaultValue={val} onChange={update}>
			...
			<ResponsiveControl label="Espacement" value={p.gap} onChange={(v) => u('gap', v)}>
				{(val, bp, update) => (
					<LFW {...lockProps} fieldKey={`gap.${bp}`} label="" type="select" defaultValue={val} onChange={update}>
