...
export default function PagesEditPage({ page, translations }: Props) {
	...
	const currentTranslation = page.translations.find((t) => t.locale === activeLocale) ?? page.translations[0];
	...
	const { presence, connected, currentUserId, emit, acquireLock, releaseLock, getLock, pushDraft } = useBuilderSync({
		pageId: page.id,
			...
			<div className="flex items-center justify-between h-18 gap-3 p-3">
				<div className="flex items-center gap-3">
						...
						{(['editor', 'split', 'preview'] as PanelMode[]).map((mode) => (
							<button
								...
								className={`px-2.5 py-1.5 text-xs border-r border-edge last:border-0 transition-colors ${
									panelMode === mode ? 'bg-primary-soft text-ink-inverted' : 'text-ink-muted hover:bg-sunken'
								}`}
					...
					<CanAccess permission="templates.view">
						<Button variant="outline" fitContent name="apply-template-open" onClick={() => setApplyTemplateOpen(true)}>
				...
				<SaveButton state={saveState} saving={saving} onClick={handleSave} translations={translations} />
			</div>
									...
									<div className="flex items-center justify-between">
										...
										<span
											className={`inline-flex items-center gap-1 text-xs font-medium ${
												currentTranslation?.status === 'published' ? 'text-success' : 'text-ink-muted'
											}`}
										>
											<span
												className={`w-1.5 h-1.5 rounded-full ${STATUS_CSS[currentTranslation?.status ?? 'draft'].dot}`}
											/>
											{translations.status[(currentTranslation?.status as keyof typeof translations.status) ?? 'draft']}
										</span>
									</div>
...
function AddTranslationButton({
	...
	return (
		<div className="relative ml-auto">
			...
			>
				<span className="w-4 h-4 rounded border border-dashed border-edge flex items-center justify-center">+</span>
				{t('locale.add')}
			</button>
			{open && (
