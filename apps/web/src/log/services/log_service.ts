 ...
 */
const LEVEL_ORDER: LogLevel[] = [LogLevel.DEBUG, LogLevel.INFO, LogLevel.WARN, LogLevel.ERROR, LogLevel.FATAL];
 ...
 */
const DEBUG_NOISE_CATEGORIES: LogCategory[] = [LogCategory.API, LogCategory.DATABASE, LogCategory.PERFORMANCE];
...
export class LogService {
	...
	log(entry: LogEntry): void {
		const { message, level = LogLevel.INFO, category = LogCategory.SYSTEM, context = {}, error, metadata = {} } = entry;
	...
	private shouldPersist(level: LogLevel, category: LogCategory): boolean {
		...
		if (excludeDebugNoise && level === LogLevel.DEBUG && DEBUG_NOISE_CATEGORIES.includes(category)) {
			return false;
		}
