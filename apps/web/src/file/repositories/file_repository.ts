...
export class FileRepository extends BaseRepository {
	 ...
	 */
	async list(filters: ListFilters, pagination: PaginationFilters): Promise<ModelPaginatorContract<File>> {
		const query = File.query(this.client()).preload('folder').preload('alts').orderBy('created_at', 'desc');
	...
	async upsertAlt(fileId: number, locale: string, key: string, value: string): Promise<void> {
		const { default: FileAlt } = await import('#file/models/file_alt');
	...
	async deleteAlt(fileId: number, locale: string, key: string): Promise<void> {
		...
		await FileAlt.query(this.client()).where('file_id', fileId).where('locale', locale).where('key', key).delete();
	}
