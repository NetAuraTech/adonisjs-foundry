...
test.group('Admin REST API v1 — Roles', (group) => {
	...
	test('shows a role by id', async ({ client, assert }) => {
		...
		const res = await client.get(`/api/v1/admin/roles/${role.id}`).accept('json').bearerToken(token.value!.release());
	...
	test('show returns 404 on unknown id', async ({ client }) => {
		...
		const res = await client.get('/api/v1/admin/roles/99999').accept('json').bearerToken(token.value!.release());
	...
	test('creates a role', async ({ client, assert }) => {
		...
		const res = await client.post('/api/v1/admin/roles').accept('json').bearerToken(token.value!.release()).json({
			name: 'Guest',
			slug: 'guest',
	...
	test('delete returns 404 on unknown id', async ({ client }) => {
		...
		const res = await client.delete('/api/v1/admin/roles/99999').accept('json').bearerToken(token.value!.release());
	...
	test('roles endpoints return 403 without the required permission', async ({ client }) => {
		...
		const res = await client.get('/api/v1/admin/roles').accept('json').bearerToken(token.value!.release());
	...
	test('lists all permissions', async ({ client, assert }) => {
		...
		const res = await client.get('/api/v1/admin/permissions').accept('json').bearerToken(token.value!.release());
