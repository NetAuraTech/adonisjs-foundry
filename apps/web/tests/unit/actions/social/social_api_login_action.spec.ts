...
test.group('SocialApiLoginAction', () => {
	...
	test('execute() creates a new user and issues a token when no match exists', async ({ assert }) => {
		const action = await app.container.make(SocialApiLoginAction);
	...
	test('execute() throws UnverifiedAccountException when email matches an unverified account', async ({ assert }) => {
		const action = await app.container.make(SocialApiLoginAction);
