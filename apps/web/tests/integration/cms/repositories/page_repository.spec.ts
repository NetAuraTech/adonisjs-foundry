...
import { PageFactory, PageTranslationFactory, PageRevisionFactory } from '#factories/cms/page_factory';
...
test.group('PageRepository', () => {
	...
	test('create() inserts a new page', async ({ assert }) => {
		const { UserFactory } = await import('#factories/identity/user_factory');
