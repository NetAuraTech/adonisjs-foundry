...
export default class MaintenanceOn extends BaseCommand {
	...
	async run() {
		...
		if (!redisAvailable && !this.forceMemory) {
			this.logger.error('Cannot enable maintenance: Redis unavailable and no explicit memory override.');
		...
		const message =
			this.message || (await this.prompt.ask('Enter maintenance message:')) || 'Application is under maintenance.';
