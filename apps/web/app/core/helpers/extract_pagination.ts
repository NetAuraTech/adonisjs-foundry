 ...
 *
 * Use this helper whenever input validation matters — e.g. in controllers where
 * the client data should not be trusted. Invalid values are rejected by Vine
 ...
 *         values, negative numbers, or any constraint defined in the pagination
 *         schema).
 *
 * @example
 * ```ts
 ...
 * const result = await User.query().paginate(pagination.page, pagination.perPage)
 * ```
 */
