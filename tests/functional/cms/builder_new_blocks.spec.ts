...
test.group('Builder — new block types', (group) => {
  ...
  test('video and carousel blocks are saved through the editor save endpoint', async ({
    client,
    assert,
  }) => {
  ...
  test('the five new block types resolve and are served on the public page', async ({
    client,
    assert,
  }) => {
