...
test.group('Registration endpoint', (group) => {
  ...
  test('register: valid data redirects to the profile and creates an unverified user', async ({
    client,
    assert,
  }) => {
  ...
  test('register: a duplicate email returns a 422 with an email field error', async ({
    client,
    assert,
  }) => {
