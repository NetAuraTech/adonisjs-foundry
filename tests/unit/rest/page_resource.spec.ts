...
test.group('handlePage', () => {
  ...
  test('flashes the success message and redirects to the named route on mutating methods', async ({
    assert,
  }) => {
  ...
  test('throws a developer error when a display endpoint lacks its render declaration', async ({
    assert,
  }) => {
