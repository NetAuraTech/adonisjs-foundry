...
test.group('UploadFileAction', () => {
  test('execute() throws FileTooLargeException when file exceeds size limit', async ({
    assert,
  }) => {
  ...
  test('execute() throws InvalidExtensionException for disallowed extension', async ({
    assert,
  }) => {
    const action = await app.container.make(UploadFileAction)
