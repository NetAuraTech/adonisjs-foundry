...
test.group('SaveBlockTemplateAction', () => {
  ...
  test('execute() overwrites an existing block template when overwriteId is set', async ({
    assert,
  }) => {
    const action = await app.container.make(SaveBlockTemplateAction)
    const user = await UserFactory.create()
  ...
  test('execute() throws E_INVALID_TEMPLATE_TYPE when overwriting a page template', async ({
    assert,
  }) => {
    const action = await app.container.make(SaveBlockTemplateAction)
    const user = await UserFactory.create()
