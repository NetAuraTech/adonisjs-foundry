...
test.group('File model — resolveAlt()', () => {
  ...
  test('falls back to the default-locale keyed alt when the requested locale misses', ({
    assert,
  }) => {
  ...
  test('falls back to the keyed alt in any locale when locale and default both miss', ({
    assert,
  }) => {
    const file = makeFile([
      { locale: 'en', key: 'hero', value: 'English hero alt' },
