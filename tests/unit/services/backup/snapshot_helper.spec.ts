...
test.group('SnapshotHelper', (group) => {
  ...
  test('generateFilename() produces correctly formatted full backup filename', async ({
    assert,
  }) => {
    const { SnapshotHelper } = await import('#services/backup/snapshot_helper')
  ...
  test('generateFilename() produces correctly formatted differential filename', async ({
    assert,
  }) => {
    const { SnapshotHelper } = await import('#services/backup/snapshot_helper')
  ...
  test('manifestFilename() strips all extensions and appends .manifest.json', async ({
    assert,
  }) => {
    const { SnapshotHelper } = await import('#services/backup/snapshot_helper')
    assert.equal(
