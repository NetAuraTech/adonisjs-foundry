...
test.group('BackupPipeline', (group) => {
  ...
  test('compressAndEncrypt() runs compress, unlink dump, encrypt and stat in order', async ({
    assert,
  }) => {
    const { BackupPipeline } = await import('#services/backup/backup_pipeline')
    const logService = buildLogService()
  ...
  test('writeManifest() serializes, writes, uploads and tracks the manifest', async ({
    assert,
  }) => {
    const { BackupPipeline } = await import('#services/backup/backup_pipeline')
    const logService = buildLogService()
  ...
  test('run() returns the body result and cleans up every tracked temp file', async ({
    assert,
  }) => {
    const { BackupPipeline } = await import('#services/backup/backup_pipeline')
    const logService = buildLogService()
  ...
  test('run() keeps failing cleanup silent when a temp file is already gone', async ({
    assert,
  }) => {
    const { BackupPipeline } = await import('#services/backup/backup_pipeline')
    const logService = buildLogService()
  ...
  test('noArtifactResult() builds a success result without a filename or size', async ({
    assert,
  }) => {
    const { BackupPipeline } = await import('#services/backup/backup_pipeline')
    const logService = buildLogService()
