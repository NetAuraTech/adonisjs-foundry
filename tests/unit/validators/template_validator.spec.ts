...
test.group('Template validators', () => {
	...
	test('createTemplateValidator requires name, type, and content', async ({ assert }) => {
		await assert.rejects(() => createTemplateValidator.validate({ type: 'page', content: { blocks: [] } }));
		await assert.rejects(() => createTemplateValidator.validate({ name: 'T', content: { blocks: [] } }));
	...
	test('createBlockTemplateValidator accepts a valid single-root submission', async ({ assert }) => {
	...
	test('templatePreviewValidator requires positive id, locale, and token', async ({ assert }) => {
		await assert.rejects(() => templatePreviewValidator.validate({ id: 0, locale: 'en', token: 'x' }));
