...
test.group('LogService write-through', (group) => {
  ...
  test('explicit top-level identity fields take precedence over context keys', async ({
    assert,
  }) => {
    const actor = await UserFactory.create()
  ...
  test('security and business entries are always persisted, regardless of level', async ({
    assert,
  }) => {
  ...
  test('entries emitted inside an ambient transaction are persisted after commit', async ({
    assert,
  }) => {
