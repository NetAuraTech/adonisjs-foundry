...
test.group('PageSitemapCollector', (group) => {
  ...
  test('collect() emits /{slug} for a published subpage in the default locale', async ({
    assert,
  }) => {
  ...
  test('collect() emits /{locale}/{slug} for a published subpage in a non-default locale', async ({
    assert,
  }) => {
    const page = await PageFactory.merge({ isHomepage: false, defaultLocale: 'en' }).create()
    await PageTranslationFactory.merge({
  ...
  test('collect() emits /{locale}/ for a published homepage in a non-default locale', async ({
    assert,
  }) => {
    const homepage = await PageFactory.merge({ isHomepage: true, defaultLocale: 'en' }).create()
    await PageTranslationFactory.merge({
  ...
  test('collect() includes the published translation of a mixed-status page only', async ({
    assert,
  }) => {
    const page = await PageFactory.merge({ isHomepage: false, defaultLocale: 'en' }).create()
    await PageTranslationFactory.merge({
