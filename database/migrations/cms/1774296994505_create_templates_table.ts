...
export default class extends BaseSchema {
	...
	async up() {
		this.schema.createTable(this.tableName, (table) => {
			...
			table.integer('thumbnail_id').unsigned().nullable().references('id').inTable('files').onDelete('SET NULL');
			table.enum('type', ['page', 'block']).notNullable();
			table.string('block_type', 50).nullable().comment('Populated only when type = block, e.g. "hero"');
