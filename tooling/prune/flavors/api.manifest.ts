...
const apiManifest: FlavorManifest = {
	...
	delete: [
		...
		// `#transport/auth/controllers/api/routes`, `#transport/account/controllers/api/routes`
		// and `#transport/core/controllers/api/routes` in the start/routes.ts rewrite;
		// the health probes come from `#transport/core/health.routes`.
		...
		'apps/web/tests/functional/auth/token_consume.spec.ts',
	...
	rewrites: [
		...
		// ─── .env.example — API-token-only template with CORS first-class ────
		{
			...
			content: [
				...
				'',
				'# Queue',
				'# redis: jobs are consumed by a separate worker (node ace queue:work).',
				'# sync: jobs run inline in the calling process (no worker, no Redis).',
				'QUEUE_DRIVER=redis',
