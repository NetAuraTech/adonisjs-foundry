...
import { applyOperation } from '~/components/cms/utils/builder_reducer';
import type { BuilderOperation, BroadcastPayload, ServerBroadcastEvent, UserSession } from '#cms/types/builder';
...
interface UseBuilderSyncReturn {
	...
	acquireLock: (blockId: string, fieldKey: string) => Promise<{ acquired: boolean; lock?: LockState }>;
	...
	useEffect(() => {
		...
		function handleSseEvent(ev: any) {
			...
			if (ev.op === 'LOCK_ACQUIRED') {
				...
				setLocks((prev) => {
					const without = prev.filter((l) => !(l.blockId === ev.blockId && l.fieldKey === ev.fieldKey));
					return [
						...without,
						{
							blockId: ev.blockId,
			...
			if (ev.op === 'LOCK_RELEASED' || ev.op === 'LOCK_EXPIRED') {
				setLocks((prev) => prev.filter((l) => !(l.blockId === ev.blockId && l.fieldKey === ev.fieldKey)));
				return;
			}
