					...
					{(Object.entries(VIEWPORTS) as [Viewport, (typeof VIEWPORTS)[Viewport]][]).map(([key, vp]) => (
						<button
							...
							className={`px-2 py-1 text-xs transition-colors ${
								viewport === key ? 'bg-primary-soft text-primary-deep' : 'text-ink-muted hover:bg-sunken'
							}`}
						>
							{vp.icon}
						</button>
			...
			<div className="flex-1 flex justify-center overflow-hidden relative">
				{error && (
					<div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-canvas">
						...
						<button type="button" onClick={handleRefresh} className="text-xs text-primary-mid hover:underline">
							Retry
						</button>
					</div>
				)}
