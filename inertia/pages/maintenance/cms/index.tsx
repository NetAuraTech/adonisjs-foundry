...
import type { CmsMaintenanceTranslations } from '#types/translations'
...
interface PageProps {
  ...
  source?: 'redis' | 'memory'
  translations: CmsMaintenanceTranslations
}
            ...
            {({ processing }) => (
              <>
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <Label
                    label={config.enabled ? t('toggle.is_enabled') : t('toggle.is_disabled')}
                    htmlFor="enabled"
                  />
                </div>
